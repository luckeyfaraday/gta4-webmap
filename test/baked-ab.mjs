...
try {
  ...
  const coverage = await page.evaluate(() => {
    // Batching collapsed the per-placement materials this used to count, so the
    // unit is now the batch: sector-builder groups on whether COLOR_0 is baked
    // light or terrain blend weights, and records which on each batch material.
    let baked = 0, plain = 0, total = 0;
    const sums = [];
    const seen = new Set();
    globalThis.gta4map.scene.traverse(object => {
      if (!object.isMesh || !object.geometry?.attributes?.position) return;
      for (const material of [object.material].flat()) {
        if (!material?.userData?.batch) continue;
        total++;
        if (material.userData.batch.baked) baked++; else plain++;
      }
      const colour = object.geometry.attributes.color;
    ...
    return {
      materials: total,
      withVertexColours: baked,
      terrainExcluded: plain,
  ...
  if (!coverage.withVertexColours) throw new Error('No batch carries baked vertex lighting.');
